...
class PointwiseGenerator(IterableDataset):
    ...
    def __iter__(self):
        for i in range(self.num_data):
            ...
            item = torch.LongTensor(torch.tensor(self.items[i]))
            rating = torch.FloatTensor([torch.tensor(self.ratings[i])])

            user_id = self.users[i]
            implicit_items = torch.FloatTensor(self.implicit_matrix[user_id])

            yield user, item, rating, implicit_items
            ...
            if self.num_negative > 0:
                ...
                for neg in neg_items:
                    item = torch.LongTensor(torch.tensor(neg))
                    rating = torch.FloatTensor([torch.tensor(0)])
