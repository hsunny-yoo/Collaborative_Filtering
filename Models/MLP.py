...
from Models.config import OptimizerConfig, MLPConfig
...
class MLP(BaseModel):
    def __init__(self, input, config_train, config_opt=OptimizerConfig, config=MLPConfig):
        super().__init__()
        ...
        self.user_embedding_mlp = nn.Embedding(self.n_user, config.dim_latent)
        self.item_embedding_mlp = nn.Embedding(self.n_item, config.dim_latent)
        self.fc1 = nn.Linear(config.dim_latent*2, 32)
    ...
    def forward(self, users, items):
        embedded_users = self.user_embedding_mlp(users)
        embedded_items = self.item_embedding_mlp(items)
        concated = torch.cat([embedded_users, embedded_items], dim=-1)
