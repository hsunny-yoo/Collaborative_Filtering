...
from Models.config import OptimizerConfig, GMFConfig
...
class GMF(BaseModel):
    def __init__(self, input, config_train, config_opt=OptimizerConfig, config=GMFConfig):
        super().__init__()
        ...
        self.user_embedding_gmf = nn.Embedding(self.n_user, config.dim_latent)
        self.item_embedding_gmf = nn.Embedding(self.n_item, config.dim_latent)
        self.sum_weight_gmf = nn.Linear(config.dim_latent, 1, bias=False)
    ...
    def forward(self, users, items):
        embedded_users = self.user_embedding_gmf(users)
        embedded_items = self.item_embedding_gmf(items)
        interact = torch.mul(embedded_users, embedded_items)
        weighted_sum = self.sum_weight_gmf(interact)
